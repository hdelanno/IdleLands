...
let Player = class Player extends character_1.Character {
    ...
    moveAction() {
        ...
        if (!mapInstance || this.x <= 0 || this.y <= 0 || this.y > mapInstance.height || this.x > mapInstance.width) {
            logger_1.Logger.error('PlayerMovement', new Error(`Out of bounds for ${this.name} at ${this.mapRegion} on ${this.map}: ${this.x}, ${this.y}. Old ${oldLoc.x}, ${oldLoc.y}`), { player: this, party: this.party });
            this.moveToStart();
        }
